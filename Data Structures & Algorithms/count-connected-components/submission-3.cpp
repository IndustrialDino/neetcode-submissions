...
public:
    ...
    void addOrUpdateNode(int node_a, int node_b) {
        ...
        // simplified - from perspective of not having to worry about the if logic
        edges_simplified_[node_a].push_back(node_b); // nice and clean;
    ...
    int countComponents(int n, vector<vector<int>>& edges) {
        ...
        single values, ones with no connection, are also nodes
        */
        parseEdges(edges);
        int connected_components = 0;
        for (int node = 0; node < n; node++){
            // skip - check edges to make there's atleast one connection for the given node
