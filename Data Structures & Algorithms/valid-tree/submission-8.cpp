...
class Solution {
public:
    void printMap(const std::unordered_set<int>& data){
    ...
    void printAdj(const std::unordered_map<int ,vector<int>>& adj) {
    ...
    bool validTree(int n, vector<vector<int>>& edges) {
        ...
        */
        ...
        for (int node = 0; node < n; node++){
            ...
            if (!adjacency_map.contains(node)) return true;
    ...
    bool hasCycles(int node, int parent, const std::unordered_map<int ,vector<int>>& adjacency_map, std::unordered_set<int>& visited_nodes) {
        ...
        if (!adjacency_map.contains(node)) return false;
        const auto& neighbors = adjacency_map.at(node);
        for (const auto& neighbor: neighbors){
